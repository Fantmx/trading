# backtest_executor_v6.py
# v6 adds:
#   - Per-symbol overrides via JSON (PER_SYMBOL_OVERRIDES_PATH)
#   - End-of-run exposure summary (avg/median/pctiles/time-near-cap, open-pos stats)
#   - Default RISK_PCT bumped to 0.035 (global); still capped by MAX_GROSS_EXPOSURE
#
# Overrides supported per symbol (keys, all optional):
#   RISK_PCT, MAX_PYRAMIDS_PER_SYMBOL, PYR_REQUIRE_IMPROVE, PYR_ADD_ATR_STEP,
#   ATR_TRAIL_MULT, BE_ATR_TRIGGER,
#   PARTIAL_TP_ENABLE, PARTIAL_TP_FRAC, PARTIAL_TP_ATR_MULT,
#   SL_ATR_MULT, TP_ATR_MULT, EXIT_ON_DECAY
#
# Example overrides JSON:
# {
#   "BTCUSDT": {"ATR_TRAIL_MULT": 1.4, "MAX_PYRAMIDS_PER_SYMBOL": 6, "PYR_ADD_ATR_STEP": 0.45},
#   "ETHUSDT": {"ATR_TRAIL_MULT": 1.4, "MAX_PYRAMIDS_PER_SYMBOL": 6, "PYR_ADD_ATR_STEP": 0.45},
#   "BNBUSDT": {"ATR_TRAIL_MULT": 1.0, "MAX_PYRAMIDS_PER_SYMBOL": 3, "PYR_ADD_ATR_STEP": 0.65}
# }
#
import os, json
...
RISK_PCT_GLOBAL    = float(os.getenv("RISK_PCT", "0.035"))  # bumped default
...
ATR_TRAIL_MULT_GLOBAL = float(os.getenv("ATR_TRAIL_MULT", "1.2"))  # keep Option A/B flavor

# Pyramiding controls (global defaults; can be overridden per symbol)
MAX_PYRAMIDS_PER_SYMBOL_GLOBAL = int(os.getenv("MAX_PYRAMIDS_PER_SYMBOL", "4"))
PYR_REQUIRE_IMPROVE_GLOBAL     = os.getenv("PYR_REQUIRE_IMPROVE", "1") not in ("0","false","False","no","NO")
PYR_ADD_ATR_STEP_GLOBAL        = float(os.getenv("PYR_ADD_ATR_STEP", "0.50"))

# Partial take-profit (global defaults; can be overridden)
...
def maybe_exit_for_symbol(sym: str, t_idx: int):
    ...
    for pos in open_pos[sym]:
        ...
        # symbol-level partial TP settings
...
def maybe_open_for_symbol(sym: str, t_idx: int):
    ...
    # Exposure cap (uses global cap)
    if _gross_exposure_frac() + float(_ovr(sym, "RISK_PCT", RISK_PCT_GLOBAL)) > MAX_GROSS_EXPOSURE:
        return
    ...
    risk_pct = float(_ovr(sym, "RISK_PCT", RISK_PCT_GLOBAL))
    usd_risk = equity * risk_pct
    if usd_risk <= 0: return
    qty = usd_risk / entry_px

    # symbol overrides for SL/TP and partial trigger
