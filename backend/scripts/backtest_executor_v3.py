# backtest_executor_v3.py
# Adds MAX_GROSS_EXPOSURE cap and optional signal re-check to hold winners longer.
# Keeps v2 features: confidence band, trend filter, per-symbol thresholds, blacklist/allowlist.
#
# ENV (key ones):
#   START_EQUITY=1000
#   RISK_PCT=0.03
#   MAX_CONCURRENT=8
#   MAX_GROSS_EXPOSURE=0.30    # fraction of equity (0.30 = 30%)
#   THRESH_LONG=0.56
#   THRESH_SHORT=0.44
#   TREND_FILTER=1
#   TREND_LEN=200
#   HOLD_HOURS=4
#   RECHECK_SIGNAL=1            # if 1, re-evaluate signal each bar for open positions
#   EXIT_ON_DECAY=0             # if 1, exit immediately when signal decays into neutral (or flips)
#   PER_SYMBOL_THRESH_PATH=per_symbol_thresholds.json
...
RISK_PCT           = float(os.getenv("RISK_PCT", "0.03"))     # user wants 3%
HOLD_HOURS         = int(os.getenv("HOLD_HOURS", "4"))        # default 4
...
MAX_CONCURRENT     = int(os.getenv("MAX_CONCURRENT", "8"))    # user wants 8
...
def _signal_ok(sym: str, row) -> Tuple[bool, str, float, float]:
    """Return (ok, direction, proba_up, sma) for current row based on thresholds & trend."""
...
def maybe_exit(sym: str, t_idx: int):
    """Check exits using next bar hi/lo, and optionally re-check signal to extend or exit sooner."""
    global equity
    ...
    # Optional: re-check signal at 'ts' to extend or decay-exit
    if RECHECK_SIGNAL:
        cur = df[df["timestamp"] == ts]
        if not cur.empty:
            ok, direction, proba_up, _ = _signal_ok(sym, cur.iloc[0])
            # If still aligned with position, extend expiry
            if ok and ((pos["side"]=="LONG" and direction=="UP") or (pos["side"]=="SHORT" and direction=="DOWN")):
                pos["expiry_ts"] = ts + timedelta(hours=HOLD_HOURS)
            elif EXIT_ON_DECAY:
                # Exit at current close if signal decays (neutral or flip)
                exit_price = float(cur.iloc[0]["close"])
    ...
    # Normal exit using next bar price info
...
def maybe_open(sym: str, t_idx: int):
    ...
    if current_expo + RISK_PCT > MAX_GROSS_EXPOSURE:
        return  # skip: would breach exposure cap

    # Need ATR on next bar for SL/TP sizing
...
for i, ts in enumerate(index[:-1]):
    ...
    # Entries (iterate symbols deterministic order)
    for s in UNIVERSE:
        maybe_open(s, i)

    equity_curve.append({"timestamp": ts, "equity": equity, "open_positions": len(open_pos), "exposure_frac": _gross_exposure_frac()})
