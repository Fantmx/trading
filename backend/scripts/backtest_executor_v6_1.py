# backtest_executor_v6_1.py
# v6_1 = v6 + autoscale-to-target exposure + override logging
#
# New env:
#   AUTOSCALE_TO_TARGET=1          # enable dynamic risk sizing toward target exposure
#   TARGET_EXPOSURE=0.28           # aim to keep gross exposure around ~28% (cap still 30%)
#   MAX_RISK_PCT_PER_TRADE=0.07    # hard ceiling per trade when autoscaling
#   MIN_RISK_PCT_PER_TRADE=0.02    # don’t open “dust” positions
#
import os, json
...
RISK_PCT_GLOBAL    = float(os.getenv("RISK_PCT", "0.035"))
HOLD_HOURS         = int(os.getenv("HOLD_HOURS", "4"))
SL_ATR_MULT_GLOBAL = float(os.getenv("SL_ATR_MULT", "1.5"))
TP_ATR_MULT_GLOBAL = float(os.getenv("TP_ATR_MULT", "2.0"))
FEE_BP             = float(os.getenv("FEE_BP", "5"))
SLIPPAGE_BP        = float(os.getenv("SLIPPAGE_BP", "5"))
MAX_CONCURRENT     = int(os.getenv("MAX_CONCURRENT", "24"))
...
PER_SYMBOL_THRESH_PATH = os.getenv("PER_SYMBOL_THRESH_PATH", "per_symbol_thresholds.tuned.json")
...
EXIT_ON_DECAY_GLOBAL = os.getenv("EXIT_ON_DECAY", "0") not in ("0","false","False","no","NO")

BREAKEVEN_TRAIL    = os.getenv("BREAKEVEN_TRAIL", "1") not in ("0","false","False","no","NO")
BE_ATR_TRIGGER_GLOBAL = float(os.getenv("BE_ATR_TRIGGER", "1.0"))
ATR_TRAIL_MULT_GLOBAL = float(os.getenv("ATR_TRAIL_MULT", "1.2"))

# Pyramiding
MAX_PYRAMIDS_PER_SYMBOL_GLOBAL = int(os.getenv("MAX_PYRAMIDS_PER_SYMBOL", "6"))
PYR_REQUIRE_IMPROVE_GLOBAL     = os.getenv("PYR_REQUIRE_IMPROVE", "1") not in ("0","false","False","no","NO")
PYR_ADD_ATR_STEP_GLOBAL        = float(os.getenv("PYR_ADD_ATR_STEP", "0.45"))

# Partial TP
PARTIAL_TP_ENABLE_GLOBAL   = os.getenv("PARTIAL_TP_ENABLE", "1") not in ("0","false","False","no","NO")
PARTIAL_TP_FRAC_GLOBAL     = float(os.getenv("PARTIAL_TP_FRAC", "0.5"))
PARTIAL_TP_ATR_MULT_GLOBAL = float(os.getenv("PARTIAL_TP_ATR_MULT", "1.5"))

# Per-symbol overrides
PER_SYMBOL_OVERRIDES_PATH = os.getenv("PER_SYMBOL_OVERRIDES_PATH", "")

# Autoscale toward target exposure
AUTOSCALE_TO_TARGET          = os.getenv("AUTOSCALE_TO_TARGET", "1") not in ("0","false","False","no","NO")
TARGET_EXPOSURE              = float(os.getenv("TARGET_EXPOSURE", "0.28"))
MAX_RISK_PCT_PER_TRADE       = float(os.getenv("MAX_RISK_PCT_PER_TRADE", "0.07"))
MIN_RISK_PCT_PER_TRADE       = float(os.getenv("MIN_RISK_PCT_PER_TRADE", "0.02"))
...
def _load_overrides(path: str) -> Dict[str, dict]:
    if not path: return {}
    p = Path(path)
    ...
    try:
        raw = json.loads(p.read_text())
        if isinstance(raw, dict):
            return {k.upper(): (v if isinstance(v, dict) else {}) for k,v in raw.items()}
    except Exception as e:
        print(f"[WARN] Failed to read overrides: {e}")
    return {}
...
OVERRIDES = _load_overrides(PER_SYMBOL_OVERRIDES_PATH)
if OVERRIDES:
    keys = sorted(set(OVERRIDES.keys()) & set(UNIVERSE))
    print(f"[BT] Overrides loaded for {len(keys)} symbols:", keys)
else:
    print("[BT] Overrides loaded: 0 (PER_SYMBOL_OVERRIDES_PATH not set or file empty)")
...
def _ovr(sym: str, key: str, default_val):
    o = OVERRIDES.get(sym.upper(), {})
    return o.get(key, default_val)

def _exit_on_decay(sym: str):
    return bool(_ovr(sym, "EXIT_ON_DECAY", EXIT_ON_DECAY_GLOBAL))
...
def _update_trailing(sym: str, pos: dict, cur_row):
    side = pos["side"]
    ...
    ATR_TRAIL_MULT = float(_ovr(sym, "ATR_TRAIL_MULT", ATR_TRAIL_MULT_GLOBAL))
    BE_ATR_TRIGGER = float(_ovr(sym, "BE_ATR_TRIGGER", BE_ATR_TRIGGER_GLOBAL))
...
def maybe_exit_for_symbol(sym: str, t_idx: int):
        ...
        if not cur.empty:
            ...
            for pos in open_pos[sym]:
                if ok and ((pos["side"]=="LONG" and direction=="UP") or (pos["side"]=="SHORT" and direction=="DOWN")):
                    ...
                    continue
                _update_trailing(sym, pos, row_cur)
                new_list.append(pos)
            open_pos[sym] = new_list

    row_next = df[df["timestamp"] == t_next]
    if row_next.empty or sym not in open_pos or not open_pos[sym]: return
    r = row_next.iloc[0]
    ...
    for pos in open_pos[sym]:
        side = pos["side"]; entry_px = pos["entry_px"]; qty = pos["qty"]
        PARTIAL_TP_ENABLE = bool(_ovr(sym, "PARTIAL_TP_ENABLE", PARTIAL_TP_ENABLE_GLOBAL))
        PARTIAL_TP_FRAC   = float(_ovr(sym, "PARTIAL_TP_FRAC", PARTIAL_TP_FRAC_GLOBAL))
...
def maybe_open_for_symbol(sym: str, t_idx: int):
    ...
    MAX_PYRAMIDS_PER_SYMBOL = int(_ovr(sym, "MAX_PYRAMIDS_PER_SYMBOL", MAX_PYRAMIDS_PER_SYMBOL_GLOBAL))
    ...
    if current_expo >= MAX_GROSS_EXPOSURE: return

    # Pyramiding improvement test
    PYR_REQUIRE_IMPROVE = bool(_ovr(sym, "PYR_REQUIRE_IMPROVE", PYR_REQUIRE_IMPROVE_GLOBAL))
    if PYR_REQUIRE_IMPROVE and sym_count > 0:
        ...
        step = float(_ovr(sym, "PYR_ADD_ATR_STEP", PYR_ADD_ATR_STEP_GLOBAL))
        if direction == "UP":
            if price_now < last_entry_px + step * atr: return
        else:
            if price_now > last_entry_px - step * atr: return
    ...
    # Base risk from overrides or global
    risk_base = float(_ovr(sym, "RISK_PCT", RISK_PCT_GLOBAL))

    # Autoscale toward target exposure
    risk_eff = risk_base
    if AUTOSCALE_TO_TARGET:
        slack_to_target = max(0.0, TARGET_EXPOSURE - current_expo)  # fraction
        risk_eff = max(risk_base, min(slack_to_target, MAX_RISK_PCT_PER_TRADE))
        risk_eff = max(MIN_RISK_PCT_PER_TRADE, min(risk_eff, MAX_RISK_PCT_PER_TRADE))
        risk_eff = min(risk_eff, max(0.0, MAX_GROSS_EXPOSURE - current_expo))

    if risk_eff < MIN_RISK_PCT_PER_TRADE or risk_eff <= 0:
        return

    usd_risk = equity * risk_eff
    qty = usd_risk / entry_px

    SL_ATR_MULT = float(_ovr(sym, "SL_ATR_MULT", SL_ATR_MULT_GLOBAL))
    TP_ATR_MULT = float(_ovr(sym, "TP_ATR_MULT", TP_ATR_MULT_GLOBAL))
    PARTIAL_TP_ENABLE = bool(_ovr(sym, "PARTIAL_TP_ENABLE", PARTIAL_TP_ENABLE_GLOBAL))
    PARTIAL_TP_ATR_MULT = float(_ovr(sym, "PARTIAL_TP_ATR_MULT", PARTIAL_TP_ATR_MULT_GLOBAL))
...
# ---- Exposure summary ----
if not eq_df.empty:
    exp = eq_df["exposure_frac"] * 100.0
    opn = eq_df["open_positions"]
    cap = MAX_GROSS_EXPOSURE * 100.0
    print("[BT][Exposure] Avg:", f"{exp.mean():.2f}%",
          "| Median:", f"{exp.median():.2f}%",
          "| P90:", f"{exp.quantile(0.90):.2f}%",
          "| P95:", f"{exp.quantile(0.95):.2f}%",
          "| Max:", f"{exp.max():.2f}%")
    near90 = (exp >= 0.90*cap).mean()*100.0
    near100 = (exp >= cap*0.99).mean()*100.0
    print("[BT][Exposure] Time ≥ 90% cap:", f"{near90:.2f}%",
          "| Time ≥ 99% cap:", f"{near100:.2f}%",
          "| Avg open pos:", f"{opn.mean():.2f}",
          "| Max open pos:", f"{opn.max():.0f}")
