...
def load_symbol_df(symbol: str) -> pd.DataFrame:
    ...
    # Derived features (safe divisions)
    df["atr_norm"] = df["atr_14"] / df["close"]

    band_width = (df["bb_upper"] - df["bb_lower"])
    # Avoid divide-by-zero
    band_width_safe = band_width.replace(0, pd.NA)

    df["bb_width"] = band_width_safe / df["close"]
    df["bb_pctb"]  = (df["close"] - df["bb_lower"]) / band_width_safe
